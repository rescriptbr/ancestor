...
function Make(Maker) {
  ...
  var createResponsiveStylesByProp = function createResponsiveStylesByProp(value, defaultStyles, mapper) {
    var reducer = function reducer(currentStyles, value) {
      var parsed = Curry._1(Maker.unboxBreakpointValue, value);

      return mediaQuery(currentStyles, value, Curry._1(mapper, parsed));
    };

    return Belt_Option.getWithDefault(Belt_Option.map(value, function (values) {
      return Belt_Array.reduce(values.sort(sortBySize), "", reducer);
    }), defaultStyles);
  };

  var make$2 = function make$2(values) {
    return function (param) {
      return values.reduce(function (styles, prop) {
        var transformer = prop.transformer;
        return Belt_Option.getWithDefault(Belt_Option.map(prop.values, function (values) {
          return Belt_Array.reduce(values.sort(sortBySize), "", function (currentStyles, value) {
            var parsed = Curry._1(Maker.unboxBreakpointValue, value);

            return mediaQuery(currentStyles, value, Curry._1(transformer, parsed));
          });
        }), prop.defaultStyles);
      }, param);
    };
  ...
  return {
    ...
    createResponsiveStylesByProp: createResponsiveStylesByProp,
...
//# sourceMappingURL=main.5aea68e34211da6a25b9.hot-update.js.map